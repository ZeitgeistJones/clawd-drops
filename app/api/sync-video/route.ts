...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { videoUrl, audioUrl, beat, mode, songName, moment } = body
    ...
    if (mode === 'song') {
      ...
      content = 'Download this video: ' + videoUrl + ' and this audio: ' + audioUrl + ' then sync them together so the peak visual moment lands at ' + (beat?.peak || 3.5) + ' seconds. Export as MP4 and return only the final URL.'
    }
