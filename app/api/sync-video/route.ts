...
export async function POST(req: NextRequest) {
  try {
    ...
    const taskRes = await fetch(`${MANUS_BASE}/v2/task.create`, {
      ...
      body: JSON.stringify({
        input: `You are a video sync specialist. Sync this video to the audio with frame-perfect timing.
...
No explanations. Sync it and return the MP4 URL.`,
    ...
    // Poll until stopped
    let finalVideoUrl = videoUrl // fallback to raw video
    for (let i = 0; i < 60; i++) {
    ...
    // Always return something — fall back to the raw video URL
    const { videoUrl } = await req.json().catch(() => ({ videoUrl: null }))
    return NextResponse.json({ videoUrl, error: err.message })
  }
}
